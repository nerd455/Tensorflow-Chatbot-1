...
def get_convers():
    ...
    with open(file_path, 'r') as f:
        for line in f.readlines():
            ...
            if len(parts) == 4:
                convo = []
                for word in parts[3][1:-2].split(', '):
                    convo.append(word[1:-1])
                convers.append(convo)
    return convers


def question_answers(id2line, convers):
    questions, answers = [], []
    for convo in convers:
...
def prepare_raw_data():
    ...
    id2line = get_lines()
    convers = get_convers()
    questions, answers = question_answers(id2line, convers)
